<?php
//http://localhost/exo/J13-PROJET-LINK/dbdraft.php
require "connexionbdd.php";
/*
function insertUser($name, $firstname, $email, $login, $psw){
    global $conn;
    $stmt = $conn->prepare("INSERT INTO user (name, firstname, email, login, psw) VALUES (?,?,?,?,?)");
    $stmt -> bind_param("sssss",$name, $firstname, $email, $login, $psw); 
    $stmt ->execute();
    $stmt -> close();
}
insertUser("Mareau", "Christian", "christian.mareau@gmail.com","login","psw");

function insertBoard($name, $remark, $access, $iduser){
    global $conn;
    $stmt = $conn->prepare("INSERT INTO board (name, remark, access, iduser) VALUES (?,?,?,?)");
    $stmt -> bind_param("sssi",$name, $remark, $access, $iduser); 
    $stmt ->execute();
    $stmt -> close();
}
insertBoard("Les liens du bonheurs", "Pratico-ludique", "private",3);
*/
/*
...
*/
/*
function insertBoardLink($url, $name, $remark, $icone, $numorder, $idboard){
    global $conn;
    $stmt = $conn->prepare("INSERT INTO board_link (url, name, remark, icone, numorder, idboard) VALUES (?,?,?,?,?,?)");
    $stmt -> bind_param("ssssii",$url, $name, $remark, $icone, $numorder, $idboard); 
    $stmt ->execute();
    $stmt -> close();
}
insertBoardLink("https://fr.wikipedia.org/wiki/Lapin", "Le lapin", "Un animal qui gagne à être mangé","",0,4);
insertBoardLink("https://fr.wikipedia.org/wiki/Cerf_élaphe", "Le cerf", "A ne pas confondre avec l'élaphant","",0,4);
*/
